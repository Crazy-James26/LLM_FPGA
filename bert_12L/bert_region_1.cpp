    ...
    #pragma HLS DATAFLOW
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          ap_int<32> outp0_dp = outp_temp.range(31, 0) + bias[(jj * block_size_b + j) * 2];
          ap_int<32> outp1_dp = outp_temp.range(63, 32) + bias[(jj * block_size_b + j) * 2 + 1];
...
void Bert_layer_dataflow_region_1(
  ...
  #pragma HLS array_partition variable=buf2 cyclic factor=2
  #pragma HLS array_partition variable=buf4 cyclic factor=2
  #pragma HLS array_partition variable=buf6 cyclic factor=2
  ...
  Linear_layer_qkv(inp_k, block_wk_loader, buf2, buf18, outp_k);  // L458
  Linear_layer_qkv(inp_v, block_wv_loader, buf4, buf19, outp_v);  // L459
  Linear_layer_qkv(inp_q, block_wq_loader, buf6, buf20, outp_q);  // L460
}
