    ...
    #pragma HLS DATAFLOW
      ...
      #pragma HLS pipeline II=1
        ...
        l_bias_scale_i: for (int i = 0; i < block_size_a; i++) {    // L40
          ...
          ap_int<32> outp0_dp = outp_temp.range(31, 0) + bias[(jj * block_size_b * 2 + j) * 2];
          ap_int<32> outp1_dp = outp_temp.range(63, 32) + bias[(jj * block_size_b * 2 + j) * 2 + 1];
          converter_t outp0;
          converter_t outp1;
...
void Bert_layer_dataflow_region_3(
    ...
    #pragma HLS array_partition variable=buf10 cyclic factor=2
    #pragma HLS array_partition variable=buf12 cyclic factor=2
    ...
    #pragma HLS STREAM variable=inp_res1 depth=32*768
    ...
    #pragma HLS STREAM variable=outp_gelu depth=3072
    ...
    Linear_layer_ds1(inp_ds1, block_w_ds1_loader, buf10, buf26, outp_ds1);
    Gelu_layer(outp_ds1, buf27, outp_gelu); 
    Linear_layer_ds2(outp_gelu, block_w_ds2_loader, buf12, buf28, outp_ds2); 
